...
\date{package version 0.1}
\maketitle 

\section{Introduction}

The package \textbf{hypred} (the name stands for \textbf{hy}brid
\textbf{pred}iction) is intended for simulating high density genomic
data. Its focus is on genomic applications in applied genetics, namely
genomic prediction and selection \cite{meuwissen} in hybrid maize
breeding , but it should be useful in related fields as well.
...
\item[Marker loci] An arbitrary number of marker loci can be assigned
  ...
  over the chromosome) or explicitly set by the user.
...
\begin{equation}
  \sum_{l} 2pq\big( a + d(q - p)\big)^2 + (2pq)^2 \notag
\end{equation} 
